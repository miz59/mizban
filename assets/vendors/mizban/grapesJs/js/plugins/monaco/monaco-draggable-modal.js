export function setupDraggableModal(modal) {
    function createDraggableWrapper() {
        ...
        wrapper.style.left = 'var(--monaco-left-default)';
        wrapper.style.top = 'var(--monaco-top-default)';
...
function setupDragging(header, wrapper) {
    ...
    function drag(e) {
        ...
        if (e.type.includes("touch")){
            deltaX = ((e.touches[0].clientX - startX) / window.innerWidth) * 100;
            deltaY = ((e.touches[0].clientY - startY) / window.innerHeight) * 100;
        }
        else{
            deltaX = ((e.clientX - startX) / window.innerWidth) * 100;
            deltaY = ((e.clientY - startY) / window.innerHeight) * 100;
        }
