...
export function makeWrapperResizable(wrapper) {
    ...
    function resizeHandle(handle, e){
        isResizing = true;
    ...
    wrapper.querySelectorAll('.resize-handle').forEach(handleElement => {
        handleElement.addEventListener('mousedown', function(e){
            resizeHandle(handleElement, e);
        });
    ...
    const onMouseMove = (e) => {
        ...
        if (e.type.includes("touch")){
            dX = ((e.touches[0].clientX - startX) / window.innerWidth) * 100;
            dY = ((e.touches[0].clientY - startY) / window.innerHeight) * 100;
        }
        else{
            dX = ((e.clientX - startX) / window.innerWidth) * 100;
            dY = ((e.clientY - startY) / window.innerHeight) * 100;
        }
